...
(with-eval-after-load 'evil-collection
  (add-hook 'shell-mode-hook
	    (lambda ()
	      ...
	      (evil-define-key 'insert shell-mode-map (kbd "TAB") 'comint-dynamic-complete)
  ...
  (defun my/copy-to-windows-clipboard (beg end)
    "Copy region to the Windows clipboard using clip.exe."
    (interactive "r")
    (shell-command-on-region beg end "clip.exe")
    (message "Copied to clipboard"))

  (define-key evil-visual-state-map (kbd "C-y") 'my/copy-to-windows-clipboard)
  ...
  (custom-set-variables
   ...
   '(package-selected-packages
     '(ivy ## rgb rg ripgrep projectile deadgrep typescript-mode pbcopy solidity-mode magit ligature org-bullets key-chord evil tree-sitter-langs rust-mode lsp-mode go-mode evil-collection))
